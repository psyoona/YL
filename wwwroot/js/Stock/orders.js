...
StockManager.prototype.loadOrders = function () {
	webServer.getData('/Stock/GetOrders', null, (response) => {
		...
		orders.forEach(o => {
			...
			switch (o.orderStatus) {
				...
				default: statusBadge = `<span class="badge">${stockManager.escapeHtml(o.orderStatus)}</span>`;
			}

			html += `<tr>
				<td data-label="주문번호">${stockManager.escapeHtml(o.orderNo || '-')}</td>
				<td data-label="종목">${stockManager.escapeHtml(o.stockName)} <small style="color:var(--text-muted)">${stockManager.escapeHtml(o.stockCode)}</small></td>
				...
				<td data-label="일시">${stockManager.formatDate(o.createdAt)}</td>
			</tr>`;
		});
